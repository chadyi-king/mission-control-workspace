...
class RedisBackbone:
    def __init__(self, redis_url: str):
        ...
        try:
            ...
            fakeredis = _load_fakeredis()
            if fakeredis is None:
                raise
            if _FAKE_SERVER is None:
                _FAKE_SERVER = fakeredis.FakeServer()
            self.redis = fakeredis.FakeRedis(server=_FAKE_SERVER, decode_responses=True)
    ...
    def claim_stale(self, stream: str, group: str, consumer: str, min_idle_ms: int, count: int = 20):
        """Claim stale messages from a stream consumer group.

        Uses XAUTOCLAIM when available (Redis >= 6.2). Falls back to
        XPENDING + XCLAIM for older Redis versions.
        """
        reclaimed = []
        try:
            next_cursor = "0-0"
            ...
            text = str(exc).lower()
            if "unknown command" not in text or "xautoclaim" not in text:
                raise

            # Redis server or client does not support XAUTOCLAIM semantics here.
            # Skip stale claiming instead of crashing executor; new messages are
            # still consumed through XREADGROUP in run loop.
            return []
...
def publish_event(backbone: RedisBackbone, stream: str, event_type: str, data: Dict) -> str:
    """
    Helper to publish a structured event to a Redis stream using RedisBackbone.publish_stream.
    Returns the stream message id.
    """
    payload = event_payload(event_type, data)
    if backbone is None:
        return ""
    return backbone.publish_stream(stream, payload)
